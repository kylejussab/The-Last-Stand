...
func _on_continue_button_pressed() -> void:
	if SaveManager.has_holdout_save():
		...
		Curtain.change_scene("res://holdout/battle/holdoutBattle.tscn", 1.0, 0.75) # wait .75 seconds while the screen is black for scene load
		AudioManager.stop_music(2.5)
...
func _update_stats_screen() -> void:
	...
	for i in range(1, 4):
		...
		if i <= sortedKeys.size():
			...
			var texturePath = "res://holdout/outro/icons/mvp/" + cardKey + ".png"
	...
	for i in range(1, 4):
		...
		var texturePath = "res://holdout/outro/icons/mods/" + modKey + ".png"

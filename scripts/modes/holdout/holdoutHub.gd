...
var cardHeadingFont = preload("res://assets/fonts/Black.tres")
var cardBodyFont = preload("res://assets/fonts/Semibold.tres")
...
func _change_opponent_avatar_expression(expression: String) -> void:
	$OpponentBox/OpponentHead.texture = load("res://assets/arenaHeads/" + Database.AVATARS[HoldoutStats.currentOpponent]["name"].get_slice(" ", 0) + expression + ".png")
